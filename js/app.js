(() => {
    ...
    (() => {
        ...
        if (window.matchMedia("(min-width: 991.98px)").matches) modules_flsModules.mousePrlx = new MousePRLX({});
        let formValidate = {
            getErrors(form) {
                let error = 0;
        ...
        function rangeInit() {
            const priceSlider = document.querySelector("#range"), estimatedCalc = document.querySelector("#estimated-calc"), loanCalc = document.querySelector("#loan-calc");
            if (priceSlider) {
            ...
            if (estimatedCalc) {
                nouislider.create(estimatedCalc, {
                    start: 3e3,
                    connect: true,
                    range: {
                        min: [ 0 ],
                ...
                let slider1Value = document.querySelector("#estimated-price");
                estimatedCalc.noUiSlider.on("update", (function(values, handle) {
                    slider1Value.innerHTML = "$ " + values[handle];
                }));
            }
            if (loanCalc) {
                nouislider.create(loanCalc, {
                    start: 36,
                    connect: true,
                    range: {
                        min: [ 0 ],
                ...
                let slider1Value = document.querySelector("#loan-period");
                loanCalc.noUiSlider.on("update", (function(values, handle) {
                    slider1Value.innerHTML = values[handle];
                }));
            }
        }
        ...
        function initSliders() {
            if (document.querySelector(".watches-hero__body")) new core(".watches-hero__body", {
            ...
            if (document.querySelector(".images-product__slider")) {
                ...
                new core(".images-product__slider", {
                    modules: [ Navigation, Thumb, Parallax, EffectFade ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                    spaceBetween: 0,
                ...
                if (breakpoint.matches) thumbsSlider.destroy(false, false);
            }
            if (document.querySelector(".sell__body")) new core(".sell__body", {
...
***************************************************************************** */
        ...
        if (document.querySelector(".form__label-file")) {
            var maxFileSize = 2 * 1024 * 1024;
            var queue = {};
            $("form#uploadImages");
            var imagesList = $("#uploadImagesList"), maxFiles = 3, errorF = $(".file__error");
            var itemPreviewTemplate = imagesList.find(".item.template").clone();
            itemPreviewTemplate.removeClass("template");
            imagesList.find(".item.template").remove();
            $("#formImage").on("change", (function() {
                var files = this.files;
                let cnt = $("#uploadImagesList li").length;
                if (cnt <= `${maxFiles}`) for (var i = 0; i < files.length; i++) {
                    var file = files[i];
                    ...
                    preview(files[i]);
                } else errorF.addClass("_error-active");
                this.value = "";
            }));
            function preview(file) {
                ...
                reader.addEventListener("load", (function(event) {
                    ...
                    itemPreview.find(".file__preview img").attr("src", event.target.result);
            ...
            $("#drop-files").on("drop", (function(e) {
                ...
                if (files.length <= maxFiles) preview(files); else {
                    alert("Вы не можете загружать больше " + maxFiles + " изображений!");
                    files.length = 0;
                    return;
                }
            }));
            imagesList.on("click", ".file__del", (function() {
                const activeLink = $("._error-active");
                let cnt = $("#uploadImagesList li").length;
                if (1 === cnt) document.querySelector("#file-drop").classList.remove("_hide");
                activeLink ? activeLink.removeClass("_error-active") : null;
                var item = $(this).closest(".item"), id = item.data("id");
                delete queue[id];
                item.remove();
            }));
        }
        if (document.querySelector(".clock")) {
            function clock() {
                const hourBlock = document.querySelector(".clock__hours"), minBlock = document.querySelector(".clock__minutes"), secBlock = document.querySelector(".clock__seconds"), deg = 6;
                setInterval((() => {
                    const day = new Date, hours = 30 * day.getHours(), min = day.getMinutes() * deg, sec = day.getSeconds() * deg;
                    hourBlock.style.transform = `rotateZ(${hours + min / 12}deg)`;
                    minBlock.style.transform = `rotateZ(${min}deg)`;
                    secBlock.style.transform = `rotateZ(${sec}deg)`;
                }), 0);
            }
            clock();
        }
        ...
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".language__btn")) {
                targetElement.classList.toggle("_active-link");
                document.querySelector(".language__submenu").classList.toggle("_active-block");
                e.preventDefault();
            }
            if (targetElement.closest(".product__fav")) targetElement.classList.toggle("_active");
            if (targetElement.closest(".search-form__btn")) document.querySelector(".filter__items").classList.toggle("_active-search");
            if (targetElement.closest("#formImage")) document.querySelector(".file").classList.add("_hide");
            if (targetElement.closest("[data-parent]")) {
                ...
                if (subMenu) {
                    const activeLink = document.querySelector("._submenu-link--active"), activeBlock = document.querySelector("._submenu-open");
                    if (activeLink && activeLink !== targetElement) {
                        activeLink.classList.remove("_submenu-link--active");
                        activeBlock.classList.remove("_submenu-open");
                    }
                    targetElement.classList.toggle("_submenu-link--active");
                    subMenu.classList.toggle("_submenu-open");
                } else console.log("Нет такого меню!");
                e.preventDefault();
            }
            if (targetElement.closest(".select-watch__btn")) {
                document.querySelector(".content-watch__form").classList.add("_show");
                const text = document.querySelector(".select-watch__title").dataset.text, textTitle = document.querySelector(".select-watch__title"), select = document.querySelector(".select_watch"), footerFind = document.querySelector(".content-watch__footer_find"), form = document.querySelector(".content-watch__form"), footerForm = document.querySelector(".content-watch__footer_form");
                textTitle.innerHTML = text;
                select.style.display = "none";
                footerFind.style.display = "none";
                footerForm.style.display = "block";
                form.classList.add("_show-form");
                const example = document.querySelector(".example__items"), activeLink = document.querySelector("._transform");
                activeLink ? activeLink.classList.remove("_transform") : null;
                example.classList.add("_transform-finish");
            }
        }
        window["FLS"] = false;
        isWebp();
